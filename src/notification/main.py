"""
Notification Agent A2A Server
Handles transactional message assembly and multi-channel dispatch (Email, SMS, Push)
"""
...
from src.notification.utils.agent_executor import NotificationAgentExecutor
from src.notification.utils.agent_card_builder import build_agent_card_from_meta
...
# Load agent metadata
with open(Path(__file__).resolve().parent / "agent_cards" / "notification_agent_card.json", "r", encoding="utf-8") as f:
    META = json.load(f)
...
from src.notification.agent.notification_agent import execute_agent as execute_fn
...
def build_app() -> Starlette:
    """Build A2A-compliant Starlette application for Notification Agent."""

    task_store = InMemoryTaskStore()

    agent_executor = NotificationAgentExecutor(
        agent_id=AGENT_ID,
    ...
    agent_card = build_agent_card_from_meta(
        meta=META,
        base_url=f"http://{NOTIFICATION_AGENT_HOST}:{NOTIFICATION_AGENT_PORT}",
    )
    ...
    async def register_with_orchestrator():
        ...
        payload = {
            "name": META.get("short_name", "NotificationAgent"),
            "host": NOTIFICATION_AGENT_HOST,
            "port": int(NOTIFICATION_AGENT_PORT),
        }
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting A2A Notification Agent server on {NOTIFICATION_AGENT_HOST}:{NOTIFICATION_AGENT_PORT}")
