...
from src.order.utils.agent_executor import OrderAgentExecutor
from src.order.utils.agent_card_builder import build_agent_card_from_meta
...
from src.order.agent.order_agent import execute_agent as execute_fn
...
def build_app() -> Starlette:
    ...
    agent_card = build_agent_card_from_meta(
        meta=META,
        base_url=f"http://{ORDER_AGENT_HOST}:{ORDER_AGENT_PORT}",
    )
    ...
    async def register_with_orchestrator():
        ...
        payload = {
            ...
            "host": ORDER_AGENT_HOST,
            "port": int(ORDER_AGENT_PORT),
        }
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting A2A Order Agent server on {ORDER_AGENT_HOST}:{ORDER_AGENT_PORT}")
