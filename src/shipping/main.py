"""
Shipping Agent A2A Server
Handles carrier rates, shipping labels, and tracking
"""
...
from src.shipping.utils.agent_executor import ShippingAgentExecutor
from src.shipping.utils.agent_card_builder import build_agent_card_from_meta
...
# Load agent metadata
with open(Path(__file__).resolve().parent / "agent_cards" / "shipping_agent_card.json", "r", encoding="utf-8") as f:
    META = json.load(f)

AGENT_ID = META.get("agent_id", "org.ecommerce.shipping_agent.v1")
...
from src.shipping.agent.shipping_agent import execute_agent as execute_fn
...
def build_app() -> Starlette:
    """Build A2A-compliant Starlette application for Shipping Agent."""
    ...
    agent_card = build_agent_card_from_meta(
        meta=META,
        base_url=f"http://{SHIPPING_AGENT_HOST}:{SHIPPING_AGENT_PORT}",
    )
    ...
    async def register_with_orchestrator():
        ...
        payload = {
            "name": META.get("short_name", "ShippingAgent"),
            "host": SHIPPING_AGENT_HOST,
            "port": int(SHIPPING_AGENT_PORT),
        }
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting A2A Shipping Agent server on {SHIPPING_AGENT_HOST}:{SHIPPING_AGENT_PORT}")
