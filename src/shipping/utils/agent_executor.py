...
class ShippingAgentExecutor(AgentExecutor):
    ...
    def _extract_interaction(self, ev: dict, text_payload: str) -> str:
        ...
        try:
            ...
            if isinstance(parsed, dict):
                ...
                if not value:
                    value = {"completion": "complete", "question": "request_input", "input_required": "request_input", "error": "failed", "failed": "failed"}.get(str(parsed.get("type") or "").strip().lower(), "")
    ...
    async def execute(
        ...
        try:
            ...
            task = context.current_task or new_task_from_user_message(context.message, context.task_id, context.context_id)
    ...
    async def _handle_streaming_execution(
        ...
        try:
            ...
            if final_interaction == "failed" or not final_response_text:
                await updater.failed(message=self._make_message(final_response_text or "Agent returned no final response"))
                return
    ...
    async def _handle_non_streaming_execution(
        ...
        try:
            ...
            if interaction == "request_input":
                await updater.update_status(TaskState.input_required, message=updater.new_agent_message([self._make_text_part(result_text)]), final=True)
                return
            if interaction == "failed" or not result_text:
                await updater.failed(message=self._make_message(result_text or "Agent returned no final response"))
                return
    ...
    async def cancel(
        ...
        await TaskUpdater(event_queue, task_id, context_id).update_status(TaskState.canceled, final=True)
